...
#define yellowBlinkTime 500 // 0.5 second for yellow light blink
...
void loop()
{
  ...
  if (digitalRead(eastButton) == HIGH) // request east>west traffic flow
  {
      ...
      // only continue if traffic flow is in the opposite (west) direction
    {
      ...
      delay(flowTime); // give time for traffic to flow
      digitalWrite(westGreen, LOW); // change west lights fromm green to yello to red
      digitalWrite(westYellow, HIGH);
      delay(changeDelay);
      digitalWrite(westYellow, LOW);
      ...
      digitalWrite(eastYellow, LOW);
      digitalWrite(eastRed, HIGH); // change east-facing light from red to green
